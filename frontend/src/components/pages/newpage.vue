<template>
    <div v-if="show_newpage" class="page_overlay" @click="close">
        ...
        <Card class="card" @click.stop>
            ...
            <div class="newfile">
                <p>请选择.psd文件: </p>
                <Upload multiple action="http://localhost:8000/new-page/" name="file"
                :format="['psd']" :before-upload="checkFileType"
                class="upload-files">
...
export default {
    ...
    methods: {
        async close() {
            ...
            await axios.delete('http://localhost:8000/clear-temp/');
        },
        ...
        async submit() {
            this.loading = true;
            try{
            const res = await axios.post('http://localhost:8000/init-page/');
